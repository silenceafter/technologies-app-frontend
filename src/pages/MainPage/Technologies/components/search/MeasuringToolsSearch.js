...
import { fetchData, setLimit, setPage, setSearch } from '../../../../../store/slices/lists/measuringToolsListSlice';
...
const MeasuringToolsSearch = React.memo(({ props, id, selectedValue, options, onChange, access }) => {
  ...
  const handleChange = (event, newValue) => {
    ...
    if (onOptionSelect) {
      onOptionSelect('measuringToolsCode', newValue);
    }
  };
  //
  return (
    <>
      {access ? (        
        <Autocomplete
          ...
          renderInput={(params) => (
            <TextField
              ...
              required
              fullWidth
              name='measuringToolsCode6'
              id={`measuring-tools-textfield-${props.id}`}
              /*error={!!errorValue}
        ...
        <TextField
          fullWidth
          name='measuringToolsCode6'
          id={`measuring-tools-textfield-${id}`}
          label="Код"
          placeholder={props.placeholder}
          variant="outlined"
          sx={{ backgroundColor: '#fff', borderRadius: 1 }}
